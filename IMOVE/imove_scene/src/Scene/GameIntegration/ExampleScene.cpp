...
/**
 * This class is not used in the build, but will remain in the repository for future reference.
 */

ExampleScene::ExampleScene(SceneConfiguration config) : sceneConfig(config), circles(new CircleRepository()) {}

void ExampleScene::draw(sf::RenderWindow &target) {
    // Clear the scene and color it black
    target.clear(sf::Color::Black);
    // Go over each circle in the circles repository and draw it
    circles->for_each((const std::function<void(std::shared_ptr<Circle>)> &) [&](std::shared_ptr<Circle> circle) {
                ...
                if (circles->has(id)) {
                    ...
                    circles->add(
                            std::make_shared<Circle>(
                                    id, Vector2(location.getX(), location.getY()), sf::Color::White
                            )
                    );
                }
            } else {
